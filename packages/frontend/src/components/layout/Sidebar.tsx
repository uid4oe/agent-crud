...
import { Plus, Menu, MessageSquare, ListTodo, FileText, Target, Trash2, Loader2, X, Pencil, Search } from "lucide-react";
      ...
      <div className="flex flex-col flex-1 overflow-hidden p-3 gap-4">
        <div className={cn("flex items-center", !expanded && "justify-center")}>
          {isMobile ? (
            <div className="flex items-center justify-between w-full px-1">
              <span className="font-semibold text-sm text-gray-800">TaskAI</span>
              <button
        ...
        <button
          ...
          className={cn(
            ...
            expanded ? "px-4 py-3 rounded-2xl" : "w-[44px] h-[44px] justify-center mx-auto rounded-xl"
          )}
        >
          <Plus className="h-5 w-5 shrink-0" />
          {expanded && <span className="font-medium text-sm whitespace-nowrap">New chat</span>}
        </button>
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide">
          {expanded && filteredConversations && filteredConversations.length > 0 && (
          ...
          <div className="space-y-0.5">
            {filteredConversations?.map((conv) => {
              ...
              const isDeleting = deleteConversation.variables?.id === conv.id && deleteConversation.isPending;
              const isRenaming = renamingId === conv.id;

              return (
                <div key={conv.id} className="relative group">
                  <button
                    onClick={() => !isRenaming && handleConversationClick(conv.id)}
                    className={cn(
                      "flex items-center gap-3 py-2 rounded-full transition-all duration-150 shrink-0 w-full",
                      expanded ? "px-3" : "w-10 h-10 px-0 justify-center mx-auto",
                      isActive
                  ...
                  >
                    ...
                    {expanded && (
                      isRenaming ? (
                        ...
                        <span className="text-sm truncate pr-14 text-left flex-1" dir="auto">
                          {conv.title || "New Chat"}
                        </span>
                      )
                    )}
        ...
        <div className="space-y-0.5">
          <NavLink
            ...
            className={({ isActive }) => cn(
              ...
              expanded ? "w-full px-3" : "w-10 h-10 px-0 justify-center mx-auto",
              isActive || location.pathname.startsWith(ROUTES.TASKS) ? "bg-sidebar-active text-sidebar-active-foreground" : "hover:bg-black/5 text-gray-700"
            )}
          ...
          <NavLink
            ...
            className={({ isActive }) => cn(
              "flex items-center gap-3 py-2 rounded-full transition-all duration-150 shrink-0",
              expanded ? "w-full px-3" : "w-10 h-10 px-0 justify-center mx-auto",
              isActive || location.pathname.startsWith(ROUTES.NOTES) ? "bg-sidebar-active text-sidebar-active-foreground" : "hover:bg-black/5 text-gray-700"
            )}
          ...
          <NavLink
            ...
            className={({ isActive }) => cn(
              "flex items-center gap-3 py-2 rounded-full transition-all duration-150 shrink-0",
              expanded ? "w-full px-3" : "w-10 h-10 px-0 justify-center mx-auto",
              isActive || location.pathname.startsWith(ROUTES.WELLNESS) ? "bg-sidebar-active text-sidebar-active-foreground" : "hover:bg-black/5 text-gray-700"
            )}
