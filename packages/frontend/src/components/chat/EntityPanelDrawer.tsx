		...
		<>
			...
			<button
				...
				className={cn(
					...
					isOpen && activeDomain ? "opacity-100" : "opacity-0 pointer-events-none",
				)}
				onClick={closePanel}
			...
			<div
				className={cn(
					...
					isOpen && activeDomain ? "translate-y-0" : "translate-y-full",
				)}
			>
				<div className="bg-white rounded-t-2xl shadow-lg max-h-[85vh] flex flex-col">
					...
					{activeDomain && (
						<div key={activeDomain} className="flex-1 overflow-hidden min-h-[50vh] animate-panel-fade">
